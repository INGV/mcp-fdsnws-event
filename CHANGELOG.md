...
### Release 1.8.0-dev (2026-08-13)
  - fix: restore the `mcp>=1.7.0` dependency floor in `pyproject.toml`. The 1.8.0-dev version bump rewrote it to `mcp>=1.8.0-dev`, applying this project's version to a third-party library: the floor no longer matched the API the code needs, and it excluded the mcp releases the image and the CI matrix actually install. The comment now says the line is not a project version, so the next bump leaves it alone
